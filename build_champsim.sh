...
# Check if this is a CRC2-style replacement policy
CRC2_FLAGS=""
if grep -q "crc2_wrapper.h" replacement/${LLC_REPLACEMENT}.llc_repl 2>/dev/null; then
    echo "${BOLD}Detected CRC2-style replacement policy${NORMAL}"
    CRC2_FLAGS="-DCRC2_COMPILE"
fi
...
# cp replacement/lru_crc2.llc_repl replacement/llc_replacement.cc
